...
Register → account created with `status: pending` → Pending Approval screen (polls every 30 s via `GET /api/auth/me`) → SuperAdmin acts in Admin Approvals:
- **Approve** → `status: active`, Organizer poll detects change → auto-redirected to full Organizer dashboard (no reload needed)
- **Reject** → `status: rejected` (soft-delete; account kept in DB for audit), all refresh tokens invalidated, Organizer poll detects change → auto-redirected to RejectedScreen within ≤30 s. Rejected Organizer can still log in and land on RejectedScreen. Account queryable by SuperAdmin.

Organizer notified in-app via screen content for P0; email notification is a P1 item.
