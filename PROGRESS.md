...
3. [Phase 1 — Auth Foundation](#phase-1--auth-foundation)
   - [Phase 1a — Backend Authentication Core](#phase-1a--backend-authentication-core)
   - [Phase 1b — Frontend Authentication UI](#phase-1b--frontend-authentication-ui)
   - [Phase 1c — SuperAdmin Approval Workflow](#phase-1c--superadmin-approval-workflow)
   - [Phase 1d — Dashboard Shells & Route Guards](#phase-1d--dashboard-shells--route-guards)
   - [Phase 1e — Forgot Password Flow](#phase-1e--forgot-password-flow)
   - [Phase 1f — Testing & Documentation](#phase-1f--testing--documentation)
...
|---|----------------|--------|
| 1 | All four roles can register and authenticate | ✅ Done |
| 2 | SuperAdmin can approve / reject pending Organizers | ✅ Done |
| 3 | Exhibitor / Attendee OTP verification works end-to-end | ✅ Done |
| 4 | Each role renders its designated dashboard shell after login | ✅ Done |
| 5 | Route guards enforce role-based access (frontend + backend) | ✅ Done |
| 6 | Forgot password flow completes successfully (3-step) | ✅ Done |
| 7 | Application is responsive on mobile (320px–1920px) | ✅ Done |
| 8 | Zero `window.alert` usage — all notifications via toast | ✅ Done |
| 9 | Dark / Light mode works and persists across sessions | ✅ Done |
| 10 | Token refresh works automatically before expiry | ✅ Done |
| 11 | SuperAdmin seed script documented and tested | ✅ Done |
| 12 | All unit and integration tests passing | ✅ Done |
...
**Requirements Covered:** 1.1–1.6, 2.1–2.5, 3.1–3.7, 21.1–21.6
...
| 5 | MongoDB Atlas connection | `src/config/database.ts` — connection pooling, startup ping verification |
| 6 | Environment variable configuration | `backend/src/config/env.ts` — full Zod schema validation; fails fast on startup if required vars missing |
...
**Requirements Covered:** 4.1–4.9, 5.1–5.9, 6.1–6.7, 7.1–7.6, 8.1–8.9, 9.3–9.7, 11.4–11.8, 12.1–14.8, 15.1–15.7, 23.1–23.6
...
| 11.1 | `src/middleware/auth.middleware.ts` | `authenticate` — extracts Bearer token, verifies JWT, attaches `req.user` |
| 11.2 | `src/middleware/authorize.middleware.ts` | `authorize(...roles)` factory — role-based access control, 403 on mismatch |
| 12.1–12.6 | `src/routes/auth.routes.ts` | register, verify-otp, resend-otp, login, refresh, logout, me |
...
| 15 | `src/middleware/error.middleware.ts` | Global error handler — handles JWT errors, duplicate key (409), validation (400), default 500 |
| 16 | `src/utils/asyncHandler.ts` | `asyncHandler` wraps async route handlers, passes errors to Express error handler |
| 36.1–36.3 | `src/routes/auth.routes.ts` | `POST /api/auth/forgot-password/request`, `/verify-otp`, `/reset` |
...
`GET /api/auth/me` — Requires valid access token. Returns the authenticated user's profile object. Used by the frontend during silent session restore on page reload.
...
**Status:** ✅ Complete  
**Completion Date:** 2026-08-22  
**Requirements Covered:** 9.1–9.2, 12.1–12.7, 13.1–13.8, 14.1–14.8, 16.1–16.8, 18.1–18.9, 19.1–19.6
...
| 19 | `src/services/api.ts` | Axios instance; request interceptor attaches Bearer token; response interceptor handles 401 `TOKEN_EXPIRED` → silent refresh + retry |
...
| 22.1–22.3 | `src/pages/auth/ForgotPassword/` | RequestResetPage, VerifyResetOTPPage, ResetPasswordPage |
| 23 | `src/guards/ProtectedRoute.tsx` | Loading spinner → unauthenticated redirect to `/login` → wrong-role redirect to role dashboard → render children |
| 24 | `src/App.tsx` | BrowserRouter, public routes, protected routes with `ProtectedRoute` wrapper |

---

### Phase 1c — SuperAdmin Approval Workflow

**Status:** ✅ Complete  
**Completion Date:** 2026-08-23  
**Requirements Covered:** 10.1–10.5, 11.1–11.8

#### Completed Tasks

| Task | File(s) | Description |
|------|---------|-------------|
| 26.1 | `src/routes/admin.routes.ts` | `GET /api/admin/pending-organizers` — SuperAdmin only |
| 26.2 | `src/routes/admin.routes.ts` | `PATCH /api/admin/organizers/:id/approve` — sets status to `active` |
| 26.3 | `src/routes/admin.routes.ts` | `DELETE /api/admin/organizers/:id/reject` — deletes account + all refresh tokens |
| 27 | `src/components/dashboard/PendingApprovalScreen.tsx` | Polls `GET /api/auth/me` every 30 seconds; redirects to full dashboard when status becomes `active` |
...
| 29 | `src/pages/auth/LoginPage.tsx` | Post-login routing: pending Organizer → `/dashboard/pending-approval`, active → `/dashboard/organizer` |
...
**Status:** ✅ Complete  
**Completion Date:** 2026-08-23  
**Requirements Covered:** 16.1–16.8, 17.1–17.8, 22.1–22.7
...
| 32.1–32.4 | `src/pages/dashboard/` | SuperAdminDashboard, OrganizerDashboard, ExhibitorDashboard, AttendeeDashboard — all use BentoCard, Sidebar, Header, BottomNav |
...
**Requirements Covered:** 12.1–12.7, 13.1–13.8, 14.1–14.8
...
Step 3 → POST /api/auth/forgot-password/reset
           ↓ hashes new password, invalidates ALL user refresh tokens
```
...
| `src/services/otp.service.test.ts` | OTP is 6 digits; hash ≠ plaintext; expiry is 5 minutes |
...
Full manual testing checklist: `docs/MANUAL_TESTING_CHECKLIST.md` — 73 test cases across 7 sections.
...
### 1. JWT Storage — Body + Memory (not httpOnly cookies)
...
- The original spec asked for pure memory storage, but that causes sessions to vanish on every page reload — an unacceptable UX regression for a multi-page SPA.
...
The real Resend email is **still sent** as normal — the bypass is purely additive. This lets you test the OTP flow without access to the registered email inbox.
...
**Rationale:** Rotation limits the window of token reuse. If a refresh token is stolen and used, the attacker's use of it invalidates the legitimate user's token, triggering a `TOKEN_REVOKED` error on the next legitimate refresh — detectable as a compromise signal.

### 5. SuperAdmin Seeding — Idempotent Script
...
| 2 | Dev runtime | `ts-node` | `tsx` | `tsx` compiles TypeScript ~3× faster than `ts-node` in watch mode, with no ESM compatibility issues. Drop-in replacement. |
...
The env validation in `src/config/env.ts` uses Zod and will **terminate the process** with a descriptive error if any required variable is missing or fails its constraint. This is intentional — a misconfigured server should not start silently.
...
### Test Database — `backend/.env.test`
...
The seed script creates the SuperAdmin account from environment variables. It is idempotent — safe to run multiple times.
...
- **Initial setup** — first time after configuring `backend/.env`
- **Password recovery** — update `SUPERADMIN_PASSWORD` in `.env`, rerun the script
- **Environment loss** — recreate `.env` from `.env.example`, fill credentials, rerun
...
| `DELETE` | `/api/admin/organizers/:id/reject` | Delete Organizer account + all refresh tokens |
...
**Fix:**
- Open [MongoDB Atlas](https://cloud.mongodb.com) → Network Access → confirm your current IP is whitelisted (or add `0.0.0.0/0` for development).
- Verify `MONGODB_URI` in `backend/.env` is the full connection string, including username, password, and database name.
- Ensure the Atlas cluster is not paused (free tier clusters pause after 60 days of inactivity — click **Resume** in the Atlas UI).
...
3. Open the [Resend dashboard](https://resend.com) → Emails — verify the send was attempted and check the delivery status.
4. Ensure the sending domain is verified in the Resend dashboard (Domains tab). Unverified domains fail silently.
5. If you cannot use a real inbox in development, set `DEV_OTP_BYPASS=true` in `backend/.env` — the OTP will be printed to the backend console (see Key Technical Decisions §3).
...
### 6. Token refresh loop — repeated 401 errors in the browser
...
**Fix:** Ensure `isLoading` is `true` while `checkAuthStatus` is running (it uses a `finally` block to always set `isLoading(false)`). If you see a blank page, check the browser console for React errors — a missing `AuthProvider` wrapper or a broken import is the most common cause.
...
- Run `npx tsc --noEmit` for a clean type check without emitting files — the output shows the exact files and lines.
...
2. Start the frontend with `npx vite --host` — this exposes the dev server on the local network IP.
...
3. Run `npm test` in both `backend/` and `frontend/` — all tests should pass.
