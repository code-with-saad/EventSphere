...
/**
 * VerifyResetOTPPage component - OTP verification for password reset
 * 
 * Features:
 * - Accept email from navigation state (passed from RequestResetPage)
 ...
 * - API call to POST /api/auth/forgot-password/verify-otp on submit
 * - Success: store reset token in localStorage, redirect to ResetPasswordPage
 * - Error: toast with error message (invalid OTP, expired OTP)
 * - Resend OTP: POST /api/auth/forgot-password/resend-otp with attempt tracking
...
export function VerifyResetOTPPage() {
  ...
  // Extract email from navigation state (passed from RequestResetPage)
  ...
  // Redirect to request page if no email in state
  useEffect(() => {
    if (!email) {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Success: show toast and redirect to reset password page
      showSuccess('OTP verified successfully. Please reset your password.');
  ...
  const handleResendOtp = async () => {
    ...
    if (resendCount >= 3) {
      showError('Maximum resend attempts exceeded. Please request a new password reset.');
      return;
    }
        ...
        <BentoCard>
          ...
          <div className="mb-6">
            ...
            <p className="text-text-secondary-dark">
              ...
              <span className="text-brand-primary-dark font-medium">{email}</span>
            </p>
          </div>
          ...
          <div className="mb-6 p-4 bg-bg-surface-dark border border-border-base-dark rounded-lg-token">
            <div className="flex items-center justify-between">
              ...
              <span
                className={`text-lg-token font-mono font-bold ${
